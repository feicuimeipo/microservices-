...
public class AdminUserServiceImpl implements AdminUserService {
    ...
    public Long createUser(UserCreateReqVO reqVO) {
        ...
        checkCreateOrUpdate(null, reqVO.getUsername(), reqVO.getMobile(), reqVO.getEmail(),
                reqVO.getDeptId(), reqVO.getPostIds(),DataLanguagePermission.stringValueSet(reqVO.getDataLanguages()));
        // 插入用户
    ...
    private void checkCreateOrUpdate(Long id, String username, String mobile, String email,
                                     Long deptId, Set<Long> postIds,Set<String> dataLanguageList) {
        // 校验用户存在
        checkUserExists(id);
        // 校验用户名唯一
        ...
        //数据语言角色
        checkDataLanguage(dataLanguageList);
        // 校验部门处于开启状态
    ...
    protected void checkDataLanguage(Set<String> dataLanguageList){
        if (dataLanguageList==null && dataLanguageList.size()==0){
            //TODO: 是否存在在数据语言表中
            throw exception(new ErrorCode(1002003113, "数据角色语言不能为空！"));
        }
    }
    ...
    public UserImportRespVO importUsers(List<UserImportExcelVO> importUsers, boolean isUpdateSupport) {
        ...
        importUsers.forEach(importUser -> {
            // 校验，判断是否有不符合的原因
            try {
                checkCreateOrUpdate(null, null, importUser.getMobile(), importUser.getEmail(),importUser.getDeptId(), null,DataLanguagePermission.stringValueSet(importUser.getDataLanguages()));
            ...
            if (existUser == null) {
                ...
                respVO.setDataLanguages(importUser.getDataLanguages());
                return;
            }
