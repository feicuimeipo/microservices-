...
public class OperateLogServiceImpl implements OperateLogService {
    ...
    public void createOperateLog(OperateLogCreateReqDTO createReqDTO) {
        ...
        if (StringUtils.isEmpty(logDO.getUserAgent())){
            log.warn("OperateLogCreateReqDTO.user-agent is not null!");
            logDO.setUserAgent("-");
        }

        if (logDO.getResultMsg()!=null && logDO.getResultMsg().length()>512){
            logDO.setResultMsg(logDO.getResultMsg().substring(0,500));
        }
