        ...
        function addNewRuleAndPush(rule) {
            AuthorityRuleService.addNewRule(rule).success((data) => {
                if (data.success) {
                   ...
                   window.alert('添加规则失败：' + data.msg);
                }
            }).error((data) => {
                if (data) {
                   window.alert('添加规则失败：' + data.msg);
                } else {
                   window.alert("添加规则失败：未知错误");
                }
            });
        }
        ...
        function saveRuleAndPush(rule, edit) {
            AuthorityRuleService.saveRule(rule).success(function (data) {
                if (data.success) {
                   ...
                   window.alert('修改规则失败：' + data.msg);
                }
            }).error((data) => {
                if (data) {
                   window.alert('修改规则失败：' + data.msg);
                } else {
                    window.alert("修改规则失败：未知错误");
                }
            });
        }
        ...
        function deleteRuleAndPush(entity) {
            ...
            AuthorityRuleService.deleteRule(entity).success((data) => {
                if (data.code == 0) {
                   ...
                   window.alert('删除规则失败：' + data.msg);
                }
            }).error((data) => {
                if (data) {
                   window.alert('删除规则失败：' + data.msg);
                } else {
                    window.alert("删除规则失败：未知错误");
                }
            });
        };
