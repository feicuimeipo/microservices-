                ...
                if (data.code === 0 && data.data) {
                    ...
                    if (failedClientSet.length === 0 && failedServerSet.length === 0) {
                        ...
                        if (failedClientSet) {
                         ...
                         window.alert('推送完毕。失败机器列表：' + JSON.stringify(failedSet));
                    }

                    location.reload();
                } else {
                    if (data.code === UNSUPPORTED_CODE) {
                         window.alert('该应用的 Sentinel 客户端不支持集群限流，请升级至 1.4.0 以上版本并引入相关依赖。');
                    } else {
                         window.alert('推送失败：' + data.msg);
                    }
                }
            }).error(() => {
                 window.alert('未知错误');
            ...
            ClusterStateService.applyClusterServerBatchUnbind($scope.app, ids).success((data) => {
                if (data.code === 0 && data.data) {
                    ...
                    if (failedClientSet.length === 0 && failedServerSet.length === 0) {
                         ...
                         window.alert('操作推送完毕，部分失败机器列表：' + JSON.stringify(failedClientSet));
                    }
