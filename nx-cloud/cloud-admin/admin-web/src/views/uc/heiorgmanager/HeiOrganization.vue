...
export default {
  ...
  mounted() {
    orgrequest.getAllOrgAuth().then(rep => {
      for (var i = 0; i < rep.rows.length; i++) {
        this.orgList.push({
          key: rep.rows[i].orgId,
          value: rep.rows[i].orgName + "--[" + rep.rows[i].demName + "]"
        });
      }
      this.orgManagerList = rep.rows; //将所有有权限的组织赋值到orgManagerList对象当中，后面会带到各个组件当中做权限判断
