<template>
  <div class="fullheight">
    <ht-table
      ...
      ref="htTable"
      v-if="defaultFiledShowConf.procInstId && defaultFiledShowConf.procInstId.desc"
    >
      <template v-slot:toolbar>
        <el-button-group>
          <el-button icon="el-icon-s-tools" @click="openSetDialog()">字段显示配置</el-button>
        </el-button-group>
      </template>
      ...
      <template>
        ...
        <ht-table-column
          ...
          width="140"
        />
        <ht-table-column prop="ownerName" :label="defaultFiledShowConf.ownerName.desc" width="120" />
        <ht-table-column
          prop="assigneeName"
          ...
          <template slot-scope="scope">
            <el-popover placement="right" width="400" trigger="click">
              <el-table :data="scope.row.cList">
                <el-table-column width="80" property="typeName" label="类型"></el-table-column>
                <el-table-column width="320" property="names" label="名称">
                  <template slot-scope="scope1">
                    <span class="owner-span" v-for="s in scope1.row.list" :key="s.type">
                      <el-tooltip
            ...
            <span v-if="scope.row && scope.row.assigneeName">{{scope.row.assigneeName}}</span>
          </template>
        </ht-table-column>
        <ht-table-column prop="prop1" :label="defaultFiledShowConf.prop1.desc" hidden width="120" />
        <ht-table-column prop="prop2" :label="defaultFiledShowConf.prop2.desc" hidden width="120" />
        <ht-table-column prop="prop3" :label="defaultFiledShowConf.prop3.desc" hidden width="120" />
        <ht-table-column prop="prop4" :label="defaultFiledShowConf.prop4.desc" hidden width="120" />
        <ht-table-column prop="prop5" :label="defaultFiledShowConf.prop5.desc" hidden width="120" />
        <ht-table-column prop="prop6" :label="defaultFiledShowConf.prop6.desc" hidden width="120" />
        ...
        <ht-table-column align="left" width="120" label="操作" :fixed="'right'">
          <template v-slot="{ row }">
            <el-dropdown
              size="mini"
              split-button
              v-if="row.isBpmTask == '1' && row.status != 'TRANSFORMING' && row.status != 'APPROVELINEING' &&  row.status != 'SIGNSEQUENCEING' && row.status != 'SIGNLINEING' && row.status != 'TRANSFORMEDINQU'"
              @command="handleCommand"
              @click="handleCommand({ row: row, command: 'completeTask' })"
            >
              <span>
                <i class="el-icon-tickets"></i>处理
              </span>
            ...
            <tr>
              ...
              <td>
                <eip-user-selector
                  ...
                  :config="{id:'modifyparam.userIds',account: 'modifyparam.userAccounts'}"
                />
              </td>
            </tr>
            <tr>
    ...
    >
        ...
        <table class="conf-table" cellspacing="0">
          ...
          <tbody>
            <tr v-for="c in filedShowConfList" :key="c.name">
              <th>{{c.name}}</th>
              <td>
                <ht-input size="mini" v-model="c.desc" validate="required" />
              </td>
              <td>
                <el-switch v-model="c.frontShow" active-color="#13ce66" inactive-color="#ff4949"></el-switch>
              </td>
            </tr>
          </tbody>
        </table>
      ...
      <div slot="footer" class="dialog-footer">
        <el-button type="primary" @click="saveConf" size="small">确 定</el-button>
        <el-button @click="filedConfDialogVisible = false;" size="small">取 消</el-button>
      </div>
    </el-dialog>
    <!-- 流程催办 -->
...
export default {
  components: { MessageTypeSelector, eipUserSelector, InstanceUrgent },
  data() {
    return {
      treeData: [],
      defaultProps: {
  ...
  methods: {
    ...
    confirm() {
      ...
      req
        .post(
          req.getContext().bpmRunTime + "/runtime/task/v1/setTaskExecutors",
          param
        )
        .then(response => {
    ...
    loadData(param, cb) {
      ...
      req
        .post(req.getContext().bpmRunTime + "/runtime/task/v1/list", param)
        .then(data => {
  ...
  created() {
    ...
    this.$http
      ...
      .then(function(resp) {
        if (resp.data) {
          this_.filedShowConfList = resp.data;
        } else {
          this_.filedShowConfList = JSON.parse(
            JSON.stringify(this_.defaultFiledShowConfList)
          );
