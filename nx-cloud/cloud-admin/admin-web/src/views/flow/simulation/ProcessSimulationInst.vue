  ...
  methods: {
    ...
    handleLogShow(id){
      ...
      this.$http.get(window.context.portal+ "/sys/sysLogs/v1/get/"+id).then(res=>{
        this.log = res.data || {"opeName":"日志暂未消费请稍后查看","opeContent":"日志暂未消费请稍后查看"};
      });
    },
    ...
    loadData(param, cb) {
      ...
      let defKeyParam = {property: "procDefKey", value: this.defKey, group: "quick", relation: "AND", operation: "EQUAL"};
      param.querys = param.querys.concat(defKeyParam);

      let sysCodeParam = {property: "sysCode", value: "bpmx_test_", group: "quick", relation: "AND", operation: "RIGHT_LIKE"};
    ...
    restore(id) {
      ...
      req
        .get(
          req.getContext().bpmRunTime + "/runtime/instance/v1/restore?id=" + id
        )
