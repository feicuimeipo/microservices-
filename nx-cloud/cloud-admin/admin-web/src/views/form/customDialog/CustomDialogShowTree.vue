<template>
    <el-dialog :title="title" width="1000px" :visible.sync="dialogVisible" :before-close="handleClose" :close-on-click-modal="false">
      <div style="height:500px;">
        <!-- 动态传入参数查询树 -->
        <div v-for="(item,$index) in customDialog.conditionfield"   :key="$index" >
          <span style="float: left;width:32%;padding-top: 10px;">
            <label>{{item.comment}}：</label>
            <ht-input type="text" v-if="item.type=='text'" v-model="item.paramVal" style="width:70%;"  :placeholder="'请输入'+item.comment" autocomplete="off"></ht-input>
            <ht-date v-if="item.type=='date'" v-model="item.paramVal" style="width:70%;"  :placeholder="'请输入'+item.comment" format="yyyy-MM-dd"></ht-date>
          </span>
        </div>
      </div>
