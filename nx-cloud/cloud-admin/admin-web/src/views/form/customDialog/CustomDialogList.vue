<template>
  <div class="fullheight">
      ...
      >
        ...
        <ht-form-item label="样式" label-width="120px">
          <ht-radio
            ...
            class="ht"
            :options="[{ key: 0, value: '列表' }, { key: 1, value: '树形' }]"
            :readonly="disabled"
          />
        </ht-form-item>
        <ht-form-item label="分页" v-if="treeSidebarData.style!=1" label-width="120px">
          <ht-radio
        ...
        <ht-form-item label="单选多选" label-width="120px">
          <ht-radio
            ...
            class="ht"
            :options="[{ key: 1, value: '单选' },{ key: -1, value: '多选' }]"
          />
        </ht-form-item>
        <ht-form-item label="数据来源" label-width="120px">
          <ht-radio
            ...
            class="ht"
            :readonly="disabled"
            :options="[{ key: 'dataSource', value: '数据源' },{ key: 'restful', value: 'REST接口' }]"
          />
        </ht-form-item>
        <!-- 数据源配置 -->
        <div v-show="treeSidebarData.dsType=='dataSource'">
          ...
          <ht-form-item label="选择表或视图" label-width="120px">
            ...
            >对象名称：{{treeSidebarData.objName}}&nbsp;&nbsp;所属数据源：{{treeSidebarData.dsalias}}</label>&nbsp;&nbsp;
        ...
        <div v-if="treeSidebarData.dsType=='restful'">
          ...
          <ht-form-item
            label="页号(key)"
            label-width="120px"
            v-if="treeSidebarData.requestType=='POST' && treeSidebarData.needPage && treeSidebarData.style != 1"
          >
            <ht-input
          ...
          <ht-form-item label="数据源" label-width="120px">
            <el-button type="primary" @click="showSettingDialog">设置列</el-button>
          </ht-form-item>
        </div>
      </el-form>
        ...
        <el-row :gutter="20">
          <el-col :span="7">
            ...
            >
              ...
              <el-table-column label="类型" prop="columnType" width="80" :key="3" />
              <el-table-column
                label="操作"
                width="100"
                v-if="treeSidebarData.dsType!='dataSource'"
            ...
            <el-tabs v-model="tabStatus" type="border-card">
                  ...
                  <el-table-column label="操作" width="100">
                    <template v-slot="{row,$index}">
                      <el-button
                        ...
                        type="danger"
                        icon="el-icon-delete"
                        @click="deleteSetting(treeSidebarData.displayfield,$index)"
                      >删除</el-button>
                    </template>
                  </el-table-column>
                </el-table>
                ...
                >
                  ...
                  <ht-form-item label="父ID初始值" label-width="120px">
                    <ht-input
                      ...
                      autocomplete="off"
                    ></ht-input>&nbsp;&nbsp;
                    <el-checkbox v-model="displayfieldTree.isScript">脚本</el-checkbox>
                  </ht-form-item>
                ...
                <el-table :data="treeSidebarData.conditionfield" border>
                  ...
                  <el-table-column label="默认值" width="250">
                    <template v-slot="{row}">
                      ...
                      />
                      <div
                        v-if="row.defaultType=='2' && row.dbType=='date' && row.condition=='BETWEEN'"
                      >
                        <ht-date
                          v-model="row.endDate"
                          placeholder="选择结束日期"
                  ...
                  <el-table-column label="操作" width="100">
                    <template v-slot="{row,$index}">
                      <el-button
                        ...
                        type="danger"
                        icon="el-icon-delete"
                        @click="deleteSetting(treeSidebarData.conditionfield,$index)"
                      >删除</el-button>
                    </template>
                  </el-table-column>
                </el-table>
              ...
              <el-tab-pane label="返回字段" name="return">
                <el-table :data="treeSidebarData.resultfield" border>
                  ...
                  <el-table-column prop="columnType" label="字段类型" width="100" />
                  <el-table-column label="操作" width="100">
                    <template v-slot="{row,$index}">
                      <el-button
                        size="mini"
                        type="danger"
                        icon="el-icon-delete"
                        @click="deleteSetting(treeSidebarData.resultfield,$index)"
                      >删除</el-button>
                    </template>
                  </el-table-column>
                </el-table>
              ...
              <el-tab-pane label="排序字段" name="order" v-if="treeSidebarData.dsType=='dataSource'">
                <el-table :data="treeSidebarData.sortfield" border>
                  <el-table-column prop="field" label="字段名" width="200" />
                  <el-table-column prop="comment" label="字段说明" width="200" />
                  ...
                  <el-table-column label="操作">
                    <template v-slot="{row,$index}">
                      <el-button
                        ...
                        type="danger"
                        icon="el-icon-delete"
                        @click="deleteSetting(treeSidebarData.sortfield,$index)"
                      >删除</el-button>
                    </template>
                  </el-table-column>
                </el-table>
              </el-tab-pane>
              <el-tab-pane label="自定义SQL" name="custom" :disabled="$store.state.login.currentUser.userAttrs.tenantId!=-1">
                ...
                <ht-select v-model="selectOp" :options="selectOptions" @change="changeOptions()" :props="{key:'field', value: 'comment'}"></ht-select><br/><br/>
                <el-input
                  type="textarea"
                  :validate="{ required: true }"
                  :autosize="{ minRows: 12}"
    ...
    <eip-tree-dialog ref="eipTreeDialog" :custom-dialog="customDialogPreview" :alias="aliasPreview"></eip-tree-dialog>
  </div>
</template>
...
export default {
  components: { customDialogShow, customDialogShowTree, eipTreeDialog },
  computed: {
    //删除对话框数据的URL
    deleteUrl: function() {
      ...
      if (this.tabStatus == "display") {
        for (let i in this.table.columnList) {
          ...
          if (this.treeSidebarData.style == "0") {
            let show = this.judgeAryData(this.treeSidebarData.displayfield,column);
            if(show){
        ...
        for (let i in this.table.columnList) {
          ...
          let show = this.judgeAryData(this.treeSidebarData.conditionfield,column);
          if(show){
    ...
    showDialog(action, id) {
      ...
      if (action) {
        ...
        if (action == "add") {
          ...
          this.disabled = true;
          this.title = "编辑对话框";
          this.getDialogById(id); //根据对话框信息ID查询对话框所有信息\
        }
      }
    },
    //根据对话框信息ID查询对话框所有信息
    getDialogById(id, action) {
      ...
      req.get(url).then(function(data) {
        ...
        if (action && action == "add") {
          for (let i = 0; i < this_.treeSidebarData.conditionfield.length; i++) {
