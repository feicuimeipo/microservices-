<template>
  <el-header height="48px">
    <div class="header-container header-left">
      <div class="flex">
        <div
          class="navbar-menu-top-sidebar">
          <i class="icon-cebianlan-menu-icon"/>
        </div>
        <router-link to="home" style="text-decoration: none;">
          <div class="logo-container" v-if="!showLogo">
            <i :class="isIeExplorer?'icon-x7plus-H':'icon-x7plus-H'" />
          </div>
          <div class="logo-container" v-if="showLogo">
              <a class="logo-link">
      ...
      <div class="flex header-right">
        ...
        <el-link :underline="false" :href="frontUrl" target="_blank">
          <i class="icon-monitor top-link-icon" /> 前端
        </el-link>
        <div
        ...
        <el-popover popper-class="header-popper" :visible-arrow="false" width="200" trigger="hover">
...
i.icon-x7plus-H {
  font-size: 36px;
  background: url("../assets/svg/logo.svg") left / 100% auto no-repeat;
  height: 48px;
  width: 120px;
  color: transparent;
}
...
.logo-container > i.icon-x7plus-H-ie {
  font-size: 36px;
