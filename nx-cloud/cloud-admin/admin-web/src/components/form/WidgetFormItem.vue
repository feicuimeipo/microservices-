<template>
  <div class="widget-form-item">
    ...
    <ht-form-item
      ...
      :label-height="element.options ? element.options.labelstyleHeight : ''"
    >
      ...
      <template v-if="element.ctrlType == 'property-text'">
        ...
        >
          <span v-html="element.options.textValue"></span>
        </font>
      </template>
      ...
      <template v-if="element.ctrlType == 'dialog'">
        <el-input
          :placeholder="element.options.placeholder"
          :style="{
            width: element.options.width ? element.options.width : '100%'
...
export default {
  ...
  computed: {
    imgSrc: function(state) {
      return this.getImgSrc(this.element);
    }
  },
  updated() {
    const me_ = this;
