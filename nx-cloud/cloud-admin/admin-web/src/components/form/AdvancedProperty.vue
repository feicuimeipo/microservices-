<template>
  <el-form data-vv-scope="field" size="small" inline>
    ...
    <template
      ...
      "
    >
      <ht-form-item label="统计函数" label-width="100px">
        <ht-input
          v-model="field.options.mathExp"
          readonly="readonly"
          style="width: 203px"
        >
    ...
    <template v-if="isAdvancedProperty(field.options.advancedProperty, 'isEditor')">
      <ht-form-item label label-width>
        ...
        <div v-if="!field.options.isEditor" style="width: 200px;"></div>
      </ht-form-item>
    </template>
    <!-- 是否密码框 inputType-->
    <template
    ...
    <!--字段校验规则-->
    <template v-if="field.options.validateType != undefined">
      <ht-form-item label-width="160px">
        <template slot="label">
    ...
    >
      <div>
        ...
        <code>
          ...
          <br />// _this 当前控件的vue对象 _this.inputValue 控件绑定的model值
          <br />}
    ...
    <el-dialog
      ...
      :close-on-click-modal="false"
    >
      <htEditor v-model="tooltip"></htEditor>
      <div slot="footer" class="dialog-footer">
        <el-button
          type="primary"
          @click="
