  ...
  >
    <el-form v-form>
      <ht-form-item label="分类标题" v-if="!report.id">
        <ht-input v-model="name" :validate="{required:true}" />
      </ht-form-item>
      <ht-form-item label="统计标题">
        <ht-input v-model="data.reportName" :validate="{required:true}" />
      </ht-form-item>
      <ht-form-item label="流程范围">
        <ht-radio
          :options="[{key:'1',value:'流程'},{key:'2',value:'流程分类'}]"
          v-model="data.porp.flowType"
          @change="changeFlowType"
        />
      </ht-form-item>
      <ht-form-item :label="data.porp.flowType==='1'?'流程':'流程分类'">
        <ht-input v-model="data.porp.name" placeholder="不选择时统计所有流程" />
      ...
      <ht-form-item label="部门/人员范围" label-width="120px">
        <ht-radio
          :options="[{key:'1',value:'部门'},{key:'2',value:'人员'}]"
          v-model="data.porp.orgType"
  ...
  methods: {
    ...
    getReportSettingData() {
      flow.getAct(this.id).then(data => {
        this.data = data;
        this.data.porp = JSON.parse(data.porp);
    ...
    changeOrgType() {
      this.data.porp.orgIds = "";
      this.data.porp.orgNames = "";
    },
    save() {
      if (this.report.id) {
        this.saveAct();
    ...
    saveCheck(){
      ...
      if (this.data.porp.dimension === "org" && !this.data.porp.orgIds) {
        this.$message({
          type: "warning",
          message:
