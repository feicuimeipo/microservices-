<template>
  <div class="navbar-default box-shadow flex">
    <el-page-header @back="formBack">
      <template v-slot:content>表单设计({{ formData.name }})</template>
    </el-page-header>
    <span>
      ...
      <el-button
        icon="icon-release"
        @click="publish(1)"
        v-if="formId && formData.status == 'draft' && formData.isPrint != 'Y'"
      >发布</el-button>
      <el-button
        icon="icon-release"
        @click="newVersion"
        v-else-if="formId && formData.isPrint != 'Y'"
      >发布新版</el-button>
    ...
    >
      <el-row style="height:100%">
        <el-col :span="14" style="padding:5px;height:100%; margin-top: 79px">
          <codemirror
            ref="mycode"
            v-model="formHtml"
            :options="cmOptions"
    ...
    >
      ...
      >
        <template>
          ...
          <ht-table-column prop="formId" width="200" label="表单ID" />
          <ht-table-column prop="createTime" width="200" label="创建时间" :sortable="true" />
  ...
  methods: {
              ...
              for (let k = 0; k < list[i].columns[j].list.length; k++) {
                if (
                  ...
                  ctrlTypeList.indexOf(list[i].columns[j].list[k].ctrlType) ==
                    -1 && !list[i].columns[j].list[k].options.noBindModel
                ) {
                  this.$message.error(
                    "【" +
                      list[i].desc +
    ...
    getPostData() {
      ...
      for (let i = 0; i < this.widgetForm.list.length; i++) {
        if(this.widgetForm.list[i].ctrlType == "amap"){
          amap++;
          if(amap > 1){
            this.$message({type: "error", message: "表单中只能存在一个地图控件!"});
            return;
          }
        }
        if (
              ...
              for (
                ...
                if(this.widgetForm.list[i].columns[j].list[k].ctrlType == "amap"){
                  ...
                  if(amap > 1){
                    ...
                    this.$message({type: "error", message: "已存在高德地图控件!"});
                    return;
                  }
                }
                //判断布局字段中是否嵌入了布局字段
                if (
                  ...
                  ctrlTypeList.indexOf(
                    this.widgetForm.list[i].columns[j].list[k].ctrlType
                  ) == -1 && !this.widgetForm.list[i].columns[j].list[k].options.noBindModel
                ) {
                  this.$message.error(
                    "【" +
    ...
    saveForm(params, this_) {
      if(this.clickCount > 0){
        this.$message({type: "warning", message: "操作过快，请两秒钟后再进行尝试！"});
        return false;
      }
      if (this.clickCount == 0) {
        ++this.clickCount;
    ...
    judgeWidgetFormList(widgetFormList) {
      ...
      for (var i = 0; i < widgetFormList.length; i++) {
          ...
          for (var j = 0; j < listValue.columns.length; j++) {
            ...
            for (let f = 0; f < fieldList.length; f++) {
              for (let v = 0; v < listValue.columns[j].list.length; v++) {
                if (
                  listValue.columns[j].list[v].name == fieldList[f].name &&
                  fieldList[f].target
                ) {
                  this.$message.error(
        ...
        } else if (!listValue.options.noBindModel) {
    ...
    validateField(field) {
      if (!field.options.noBindModel && field.name == "") {
        this.$message.error("请绑定表单属性");
        return false;
      }
