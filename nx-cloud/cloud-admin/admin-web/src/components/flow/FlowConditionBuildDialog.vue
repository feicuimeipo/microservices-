  ...
  <div>
    ...
    <span slot="title" style="font-size:20px;">
      ...
      <span slot="footer" class="dialog-footer">
        <el-input v-if="bindForm" type="textarea" :rows="2" readonly v-model="scriptStr"></el-input>
        <div>
          <el-button v-if="bindForm" type="primary" @click="previewCondition(false)" size="medium">预览表达式</el-button>
          <el-button  v-if="bindForm" type="primary" @click="previewCondition(true)" size="medium">确 定</el-button>
            ...
            for (var j = 0, tChild; (tChild = t.children[j++]); ) {
              if (tChild.children) {
                for (var z = 0, cAttr; (cAttr = tChild.children[z++]); ) {
                  var typeMoth =
                    cAttr.dataType == "number" ? ".getInt" : ".getString";
                  cAttr.pathStr =
