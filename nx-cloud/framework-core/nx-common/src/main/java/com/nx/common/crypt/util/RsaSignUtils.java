...
public class RsaSignUtils {
	...
	public static PublicKey loadPublicKey(String pubKeyString) {
		byte[] pubKeyStringBytes = pubKeyString.getBytes(StandardCharsets.UTF_8);
		byte[] keyBytes = Base64.decode(pubKeyStringBytes);
	...
	public static PrivateKey loadPrivateKey(String privateKeyString) {
		try {
			byte[] privateKeyStringBytes = privateKeyString.getBytes(StandardCharsets.UTF_8);
			byte[] privateKeyBytes = Base64.decode(privateKeyStringBytes);
	...
	public static byte[] encrypt(PublicKey key, byte[] plainBytes) {
		...
		try {
			...
			throw new BaseException(4003,"无此解密算法");
		} catch (NoSuchPaddingException e) {
			e.printStackTrace();
			return null;
		} catch (InvalidKeyException e) {
			throw new BaseException(4003,"解密私钥非法,请检查");
		} catch (IllegalBlockSizeException e) {
			throw new BaseException(4003,"密文长度非法");
		} catch (BadPaddingException e) {
			throw new BaseException(4003,"密文数据已损坏");
		} finally {
			try {
				if (out != null)
					out.close();
	...
	public static String decrypt(PrivateKey key, String encodedText) {
		byte[] encodedTextBytes = encodedText.getBytes(StandardCharsets.UTF_8);
    	byte[] bytes = Base64.decode(encodedTextBytes);
	...
	public static String signature(PrivateKey privateKey,String contents) {
		try {
			...
			throw new BaseException(4003,"私钥格式错误");
	...
	public static boolean verifySignature(String contents, String sign, PublicKey publicKey) {
		try {
			...
			byte[] signBytes = sign.getBytes(StandardCharsets.UTF_8.name());


			byte[] data = contents.getBytes(StandardCharsets.UTF_8.name());
			byte[] dataSignature = Base64.decode(signBytes);
