...
public class ClientWorker implements Closeable {
    ...
    public ClientWorker(final ConfigFilterChainManager configFilterChainManager, ServerListManager serverListManager,
        ...
        try{
            ...
            NxEnvironment nxEnvironment = com.nx.boot.launch.NxSpringBootApplicationBuilder.getNxEnvironment();
            serverAddr = nxEnvironment.getCloudServerAddr();
        }catch (ClassNotFoundException e){
            serverAddr = properties.getProperty(" spring.cloud.nacos.config.server-addr","");
        }

        if (serverAddr==null || serverAddr.length()==0 || !serverAddr.contains(properties.getProperty("serverAddr", ""))){
            return;
        }
