<template>
  <div>
            ...
            <el-main style="padding: 10px;height: 500px">
                <table class="table table-bordered" style="border-spacing: 0;width: 100%;max-height: 400px;overflow: initial;">
                  <tr>
                    ...
                    <td>
                      <div style="margin: 8px 0;">
                        ...
                        <FlowVarSelector  :defId="curNode.defId"   @node-click="varTreeOnConfirm" />
                      </div>
                      <div v-for="inNode in data.incomeNodes" :key="inNode.nodeId">
                        <el-link v-if="inNode.type=='SIGNTASK'"  @click="setEditorVal(inNode.nodeId,1)" type="primary">{{inNode.name}}投票通过</el-link>
                        <el-link v-if="inNode.type=='SIGNTASK'"  @click="setEditorVal(inNode.nodeId,2)" type="primary" style="margin-left: 20px;">{{inNode.name}}投票不通过</el-link>
                        <el-link v-if="inNode.type!='SIGNTASK'"  @click="setEditorVal(1,1)" type="primary">{{inNode.name}}-通过</el-link>
                        <el-link v-if="inNode.type!='SIGNTASK'"  @click="setEditorVal(1,2)" type="primary" style="margin-left: 20px;">{{inNode.name}}-反对</el-link>
                      </div>
                  ...
                  <tr v-for="outNode in data.outcomeNodes" :key="outNode.nodeId">
                    ...
                    <td class="code-mirror-td"  @click="setCurEditNodeId(outNode.nodeId)">
                      <codemirror
                        ref="mycode"
      ...
      <FlowConditionBuildDialog  @onConfirm="conditionDialogConfirm" :defId="curNode.defId" ref="flowConditionBuildDialog" />
  </div>
</template>
...
export default {
  ...
  methods: {
    ...
    handleClose: function() {
      let param ={ defId : this.curNode.defId, nodeId :this.curNode.nodeId , condition : JSON.stringify(this.data.conditions)};
      let this_ = this;
      req.post('${bpmModel}/flow/node/v1/branchConditionSave',param).then(response => {
