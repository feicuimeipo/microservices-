<template>
  <div>
    <el-form label-position="right" :label-width="labelWidth" :model="bpmForm" size="mini">
      <ht-form-item :label="title">
        <el-select v-model="bpmForm.pc.type" placeholder="请选择表单类型">
      ...
      <ht-form-item label="PC端" v-show="bpmForm.pc.type=='INNER'">
        ...
        <el-button icon="el-icon-search" @click="selectForm(bpmForm.pc)"></el-button>
        <el-button
      ...
      <ht-form-item v-show="bpmForm.pc.type=='INNER'" label="手机端">
        <el-tag v-show="bpmForm.mb.name">{{bpmForm.mb.name}}</el-tag>
        <span v-if="!bpmForm.mb.name" class="operation_text">请选择</span>
        <el-button icon="el-icon-search" @click="selectForm(bpmForm.mb)"></el-button>
        <el-button
      ...
      <ht-form-item label="设置" v-if="type !='2' || bpmForm.pc.name">
        <el-button v-if="bpmForm.pc.name" size="mini" @click="formAuth()" icon="icon-permission">授权</el-button>
        <el-button v-if="showAuthorizeSubData" size="mini" @click="authorizeSubData()" icon="icon-permission">子表数据授权</el-button>
        <el-button
    ...
    <FlowFormExtraPropSet @formPropSetConfirm="formPropSetConfirm" ref="flowFormExtraPropSet" />
    <!-- 子表数据授权设置 -->
    <FlowFormRightSubSetting ref="flowFormRightSubSetting" :nodeId="nodeId" :defId="defId" parentDefKey="local_" :flowKey="defkey"/>
  </div>
</template>
...
const FlowFormRightSubSetting = () => import("@/components/flow/FlowFormRightSubSetting.vue");
const FlowFormExtraPropSet = () => import("@/components/flow/FlowFormExtraPropSet.vue");
      ...
      if (data && data.length > 0) {
        ...
        if (
          ...
          this.$confirm(
            ...
            this.clearForm(this_.curSelectForm, function() {
