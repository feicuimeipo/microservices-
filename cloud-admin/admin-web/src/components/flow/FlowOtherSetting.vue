<template>
  <div class="flowOther-height">
    <el-card class="other-set__card" shadow="never">
      <div slot="header">
        <el-button size="mini" type="primary" icon="el-icon-check" @click="save">保存</el-button>
      </div>
      <el-scrollbar class="scrollbar-fullheight">
        <table class="form-table other-set__table" cellspacing="0" cellpadding="0" border="0">
          <tbody>
            <tr>
              <th>标题规则:</th>
              <td>
                <FlowVarSelector type="1" :defId="defId" @node-click="subjectRuleConfirm" />
                <br />
                <br />
                <ht-input
                  type="textarea"
            ...
            <tr>
              <th>通知类型:</th>
              <td>
                <ht-checkbox v-model="prop.notifyType" :options="handlerTypes" />
              </td>
            </tr>
            <tr>
              <th>测试通知类型:</th>
              <td>
                <ht-checkbox v-model="prop.testNotifyType" :options="handlerTypes" />
              </td>
            </tr>
            <tr>
              <th>参数设置:</th>
              <td>
                ...
                >跳过第一个节点</el-checkbox>
                <el-checkbox v-model="prop.allowTransTo">任务允许转办</el-checkbox>
                <el-checkbox v-model="prop.allowExecutorEmpty">允许执行人为空</el-checkbox>
                <el-checkbox v-model="prop.testStatus">是否正式</el-checkbox>
                <el-checkbox v-model="prop.showUrgentState">显示紧急状态</el-checkbox>
                <el-checkbox v-model="prop.readRevoke">允许已阅撤回</el-checkbox>
                <el-checkbox v-model="prop.showModifyRecord">显示表单留痕</el-checkbox>
              </td>
            </tr>
            <tr>
              <th>跳过类型:</th>
              <td>
                <ht-checkbox v-model="prop.skipRules" :options="skipConditionList" />
              </td>
            </tr>
            <tr>
              <th>办结抄送:</th>
              <td>
                ...
                >
                  <span class="control-label left-label">
                    <el-button type="primary" @click="setEndNotify">设置</el-button>
            ...
            <tr>
              ...
              <td>
                ...
                >
                  <span class="control-label left-label">
                    <el-button type="success" @click="urgentMailTelClick">邮件模板设置</el-button>
                ...
                >
                  <span class="control-label left-label">
                    <el-button type="warning" @click="urgentSmsTelClick">短信模板设置</el-button>
                    <i class="el-icon-warning"></i>
                  </span>
                </el-tooltip>
              </td>
              ...
              <td>
                ...
                >
                  <el-tooltip class="item" effect="dark" :content="item.title" placement="right">
                    <span class="control-label left-label" style="font-size: 14px">
                      {{item.value}}
                      <i class="el-icon-warning"></i>
                    </span>
                  </el-tooltip>
            ...
            <tr>
              <th>
                <el-tooltip class="item" effect="dark" content="外部子流程取主流程的设置参数。" placement="right">
            ...
            <tr>
              ...
              <td>
                ...
                >
                  <span class="control-label left-label">
                    <el-button type="primary" @click="$refs.flowAutoStartSetting.show();">设置</el-button>
                    <i class="el-icon-warning"></i>
                  </span>
                </el-tooltip>
              </td>
    ...
    >
      <div style="margin-bottom: 10px;">
        <el-button type="primary" size="mini" @click="addLine" icon="el-icon-plus">新 增</el-button>
      </div>
      <el-form data-vv-scope="settingSave">
        <el-table :data="procNotifyList" border ref="htTableSetting">
          <el-table-column label="人员配置">
            <template v-slot="{row,$index}">
              <table class="table table-bordered">
                ...
                <tr v-for="(userRule,userIndex) in row.userAssignRules" :key="userIndex">
                  <td>{{userIndex+1}}</td>
                ...
                <tr>
                  <td style="text-align: left;" colspan="4">
                    <el-button size="mini" @click="addUserCondition($index)">新增人员规则</el-button>
                  </td>
                </tr>
              </table>
            </template>
          ...
          <el-table-column label="操作" width="250">
            <template v-slot="{row,$index}">
              <MessageTypeSelector class="ht_ce" ref="messageTypeSelector" v-model="row.msgTypes"></MessageTypeSelector>
              <el-button
                size="mini"
                type="danger"
                icon="el-icon-delete"
    ...
    <el-dialog
      ...
      :close-on-click-modal="false"
    >
      <FlowVarSelector type="1" :defId="defId" @node-click="urgentMailTelConfirm" />
      <br />
      <br />
      <ht-input type="textarea" :rows="8" v-model="urgentMailTel" autocomplete="off"></ht-input>
      <div slot="footer" class="dialog-footer">
        <el-button
          type="primary"
          @click="dialogUrgentMailTelVisible = false;prop.urgentMailTel=urgentMailTel"
        >确 定</el-button>
        <el-button @click="dialogUrgentMailTelVisible = false;urgentMailTel=''">取 消</el-button>
      </div>
    </el-dialog>
    ...
    <el-dialog
      ...
      :close-on-click-modal="false"
    >
      <FlowVarSelector type="1" :defId="defId" @node-click="urgentSmsTelConfirm" />
      <br />
      <br />
      <ht-input type="textarea" :rows="8" v-model="urgentSmsTel" autocomplete="off"></ht-input>
      <div slot="footer" class="dialog-footer">
        <el-button
          type="primary"
          @click="dialogUrgentSmsTelVisible = false;prop.urgentSmsTel=urgentSmsTel"
        >确 定</el-button>
        <el-button @click="dialogUrgentSmsTelVisible = false;urgentSmsTel=''">取 消</el-button>
      </div>
    </el-dialog>

    <!-- 加载其他设置数据 -->
    <ht-load-data :url="loadDataUrl" context="bpmModel" @after-load-data="afterLoadData"></ht-load-data>
    <!-- 人员配置选择器 -->
...
export default {
  ...
  methods: {
    ...
    saveEndNotify() {
      const this_ = this;
      let url =
        "${bpmModel}/flow/plugins/v1/procNotifySave?defId=" + this.defId;
