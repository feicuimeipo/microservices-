<template>
  <div>
    <el-dialog :title="title" :visible.sync="dialogVisible"  v-if="dialogVisible" width="80%" appendToBody   :close-on-click-modal="false" top="8vh">
                ...
                <thead>
                  ...
                  <tbody>
                    <tr v-for="(calc,$index) in userRule.calcs" :key="$index">
                      <td>
                        <el-select size='mini' v-model="calc.pluginType" placeholder="请选择" @change="calcTypeChange(calc)">
                          <el-option v-for="item in nodeUserPluginList" :key="item.type" :label="item.title" :value="item.type">
                          </el-option>
                        </el-select>
                      </td>
                      <td>

                        <eipOrgSelector v-if="calc.pluginType=='org'" :single="true" :appendToBody="true" v-model="calc.orgName" :config="{code:'userRule.calcs['+$index+'].orgCode'}"/>
                        ...
                        <eipJobSelector v-if="calc.pluginType=='job'" :single="true" :appendToBody="true" v-model="calc.jobName" :config="{code:'userRule.calcs['+$index+'].jobCode'}"/>

                        <el-button v-show="calc.pluginType=='hrScript' ||  calc.pluginType=='cusers' ||  calc.pluginType=='customQuery'"  @click="selector($index)">选择</el-button>
                        <span style="font-size:13px" v-show="calc.pluginType=='hrScript' ||  calc.pluginType=='cusers' ||  calc.pluginType=='customQuery'">{{calc.description}}</span>

                        <el-select size='mini' v-model="calc.nodeId" placeholder="请选择" v-if="calc.pluginType=='sameNode'"  @change="changeSameNode(calc)">
                          <el-option v-for="item in nodeList" :key="item.nodeId" :label="item.name" :value="item.nodeId">
                          </el-option>
                        </el-select>
                        <el-switch  active-text="主负责人" inactive-text="所有负责人" v-model="calc.mainLeader" v-show="calc.pluginType=='depHead'" active-color="#13ce66" inactive-color="#dcdfe6">
                        </el-switch>
                        <el-tooltip class="item" effect="dark" placement="top-start"  v-show="calc.pluginType=='depHead'">
                          <div slot="content">获取发起人默认维度下的主部门，如无主部门则获取其部门列表的第一个部门，如要指定维度请使用人员脚本。<br/>一个组织只有一个主负责人，而负责人有多个</div>
                           <i v-show="calc.pluginType=='depHead'" class="el-icon-question"></i>
                        </el-tooltip>
                      </td>
                      <td>
                        <el-select v-if="userRule.nodeType=='userTask'" v-model="calc.extract" size='mini' >
                          <el-option  label="不抽取" value="no"> </el-option>
                          <el-option  label="抽取" value="extract"> </el-option>
                        </el-select>
                        <el-select v-if="userRule.nodeType=='signTask'" v-model="calc.extract" size='mini'>
                          <el-option  label="不抽取" value="no"> </el-option>
                          <el-option  label="抽取" value="extract"> </el-option>
                          <el-option  label="延迟抽取" value="secondExtract"> </el-option>
                          <el-option  label="用户组合" value="usergroup"> </el-option>
                        </el-select>
                      </td>
                      <td>
                        <el-select v-if="$index !=0" v-model="calc.logicCal" size='mini'>
                      ...
                      <td>
                        <el-button  icon="el-icon-arrow-up" @click="ArrayTool(userRule.calcs,$index,'up')"></el-button>
                        <el-button  icon="el-icon-arrow-down" @click="ArrayTool(userRule.calcs,$index)"></el-button>
                        <el-button  icon="el-icon-delete" @click="userRule.calcs.remove(userRule.calcs[$index])"></el-button>
                      </td>
                    </tr>
                  </tbody>
              </table>
              <FlowHrScriptSelector ref="flowHrScriptSelector" @scriptSelectorConfirm="scriptSelectorConfirm" append-to-body />
              <FlowCustomScriptSelector ref="flowCustomScriptSelector" @customScriptSelectorConfirm="customScriptSelectorConfirm" append-to-body />
              <FlowNodeUserRuleSelector ref="flowNodeUserRuleSelector" @customRuleSelectorConfirm="customRuleSelectorConfirm" append-to-body />
              <FlowCustomQuerySelector ref="flowCustomQuerySelector" @customQuerySelectorConfirm="customQuerySelectorConfirm" append-to-body />
            </form>
          </el-tab-pane>
     ...
     <FlowNodeCusersSelector ref="flowNodeCusersSelector" @cuserSelectorConfirm="cuserSelectorConfirm" append-to-body />
  </div>
</template>
...
const FlowCustomScriptSelector = () => import("@/components/flow/FlowCustomScriptSelector.vue");
const FlowNodeUserRuleSelector = () => import("@/components/flow/FlowNodeUserRuleSelector.vue");
const FlowCustomQuerySelector = () => import("@/components/flow/FlowCustomQuerySelector.vue");

export default {
  components: { FlowNodeCusersSelector,eipOrgSelector,eipJobSelector,eipPostSelector,FlowHrScriptSelector,FlowCustomScriptSelector,FlowNodeUserRuleSelector,FlowCustomQuerySelector},
  data() {
    return {
  ...
  buildData(conditionData,pathArr,tableArr,branchArr){
   ...
   getyIncaseNum(data){
    ...
    selector :function(index){
      ...
      if (type !='cusers' && type !='hrScript' && type != 'customQuery') {
    ...
    showDialog: function(param) {
      ...
      let this_ = this;
      req.get(req.getContext().bpmModel+"/flow/node/usercalc/v1/getNodeUserPluginList").then(function(respose) {
        ...
        this_.conditionObj={};
        if(param &&param.userRule  && param.userRule.condition){//初始化
    ...
    handleClose: function() {
      var r={};
      if(!this.userRule.calcs||this.userRule.calcs.length==0){
          this.$message.error("请设置人员！");
          return false;
      }
      if(!this.validateData(this.userRule.calcs)){
          this.$message.error("存在无效的人员设置！");
          return false;
      }
      ...
      for (const c of r.calcs) {
        if (!c.description) {
            switch(c.pluginType){
                case "org": c.description="[部门]"+c.orgName;break;
                case "position": c.description="[岗位]"+c.posName;break;
                case "job": c.description="[职务]"+c.jobName;break;
            }
        }
        if(r.description){
            if(c.logicCal=="or"){
