      ...
      <el-col :span="16">
        ...
        >
          ...
          <el-table-column prop="mg" label="合并查询" width="80">
            <template scope="scope">
              <el-checkbox v-model="scope.row.mg" />
            </template>
          </el-table-column>
          <el-table-column label="管理">
            <template slot-scope="scope">
              <el-button
                @click="addCtrlField(scope.$index, scope.row)"
                type="info"
                size="small"
                v-if="
                  scope.row.ct == 'select' ||
...
export default {
  ...
  mounted() {
    ...
    if (this.data.fields) {
      this.conditionSettingFields = [...this.data.fields];
    }
  ...
  methods: {
    ...
    addCtrlField(index, row) {
      this.editFieldIndex = index;
      this.editField = row;
      this.$refs.templateCtrlFieldDialog.showDialog(row);
    }, //回填查询字段控件设置
