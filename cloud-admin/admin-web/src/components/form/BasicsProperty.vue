  ...
  <el-form data-vv-scope="field" size="small" inline>
    ...
    <template v-if="isBasicsProperty(field.options.basicsProperty, 'choiceType')">
      ...
      <div v-if="field.options.choiceType == 'dynamic'" class="customQuery-inputs">
        ...
        <ht-form-item label label-width>
          ...
          <div v-else>
            <ht-form-item label="值" label-width="30px" class="customQuery-inputs">
              <ht-select
                clearable
                style="width:110px;"
                v-model="field.options.customQuery.valueBind"
      ...
      >
        ...
        <div slot="footer" class="dialog-footer">
          <el-button type="primary" @click="selectQueryDialog = false" size="small">返 回</el-button>
        </div>
      </el-dialog>
      <el-button
        v-if="
        ...
        <table class="form-table" cellspacing="0" cellpadding="0" border="0">
          <tbody>
            ...
            <tr v-for="(column, index) in linkage" :key="index">
              <td width="190px">
                <ht-select v-model="column.value" :options="field.options.options" />
              </td>
              <td colspan="4">
              ...
              <td>
                <el-button icon="el-icon-delete" @click="linkage.remove(column)"></el-button>
              </td>
            </tr>
          </tbody>
        </table>
    ...
    <template v-if="isBasicsProperty(field.options.basicsProperty, 'custDialog')">
      ...
      >
        ...
        <div slot="footer" class="dialog-footer">
          <el-button type="primary" @click="dialogQuery = false" size="small">返 回</el-button>
        </div>
      </el-dialog>
      <!-- 获取对话框选项列表-->
      <ht-load-data
    ...
    <!-- 公式计算 -->
    <template v-if="field.options.formulasDiyJs !== undefined">
      <ht-form-item label="公式编辑" label-width="100px" style="margin-top: 10px">
        <el-button style="min-width: 200px;" type="primary" plain @click="formulasDialog">编写公式</el-button>
      </ht-form-item>
    ...
    <template v-if="isBasicsProperty(field.options.basicsProperty, 'img')">
      <ht-form-item label-width="200px">
        ...
        <el-row v-show="field.options.isDisplay">
          <el-col :span="24">
            <img-file-upload v-model="field.options.fileJson"></img-file-upload>
          </el-col>
        </el-row>
        <el-row v-show="!field.options.isDisplay">
...
export default {
  ...
  props: ["data", "mainBoFields", "boDefData", "allBoData"],
  components: {
    HtFormItem,
    ValidataProperty,
    IconDialog,
