<template>
  <el-dialog
      ...
      <el-tab-pane label="基本信息">
        <el-form data-vv-scope="viewEdit">
          <ht-form-item label="名称" validate="required">
            <ht-input v-model="data.name" style="width:80%;"  :validate="{required:true}"/>
          </ht-form-item>
          <ht-form-item label="别名" validate="required">
            <ht-input v-model="data.alias" style="width:80%;"  v-pinyin="data.name" :disabled="data.id?true:false" :validate="{required:true}"/>
          </ht-form-item>
      ...
      <el-tab-pane label="条件字段">
        <el-table
          ...
          <el-table-column
            ...
            <template slot-scope="scope">
              <ht-select
                  :options="[{key:'EQ',value:'等于'},{key:'NE',value:'不等于'},{key:'LK',value:'相似'},{key:'LFK',value:'左相似'},{key:'RHK',value:'右相似'}]"
                  :props="{key:'key',value:'value'}"
    ...
    <template-edit ref="templateEdit" :id="this.id"/>
  ...
  export default {
    ...
    methods:{
      ...
      afterLoadEvent(){
        ...
        console.info(this.data.buttons);
