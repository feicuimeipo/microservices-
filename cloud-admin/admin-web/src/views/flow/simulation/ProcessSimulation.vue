                  ...
                  <tr v-for="(actionType,$actionTypeIndex) in testCase.actionType[item.defKey]" :key="$actionTypeIndex" >
                    ...
                    <td>
                      <ht-select v-model="actionType.actionName"
                       :options="[{key:'agree',value:'同意'},{key:'oppose',value:'反对'},{key:'reject',value:'驳回上一步'},{key:'backToStart',value:'驳回发起人'},{key:'endProcess',value:'终止流程'}]"
                        @change="selectNodeId('actionType',item.defKey,actionType,$actionTypeIndex)"
                      ></ht-select>
                      <span v-show="actionType.actionName=='reject' || actionType.actionName=='backToStart' "
                      style="display: block;padding-top: 5px;" >
                        驳回次数设置
                        <el-input-number  v-model="actionType.count" style="width:85px" :validate="{required:true}" />
                      </span>
                    </td>
                    <th colspan="2" style="text-align: center;" >
                      <el-button   icon="el-icon-delete" @click="delItem(testCase.actionType[item.defKey],$index)" ></el-button>
                    </th>
                  </tr>
                </table>
        ...
        <ht-form-item label="断点设置" prop="name" label-width="150px">
          <table style="width:100%" >
            <tr  v-for="(item,$defKeyIndex) in baseFlowInfo.defKeys" :key="$defKeyIndex">
              <th> {{item.defName}}--{{item.defKey}}</th>
              <td  colspan="4">
                <el-button @click="addItem('bpmDebugger',item.defKey)">添加断点</el-button>
                <table>
                  <tr v-for="(key,$index) in testCase.bpmDebugger[item.defKey]" :key="$index" >
                    <td>
                       <ht-select v-model="testCase.bpmDebugger[item.defKey][$index]"
                       :options="baseFlowInfo.nodeInfo[item.defKey]"
                       :props="{key:'nodeId',value:'nodeName'}"
                        @change="selectNodeId('bpmDebugger',item.defKey,key,$index)"
                      ></ht-select>
                    </td>
                    <th colspan="2" >
                      <el-button   icon="el-icon-delete" @click="delItem(testCase.bpmDebugger[item.defKey],$index)" ></el-button>
                    </th>
                  </tr>
                </table>
  ...
  <el-dialog
    ...
    :before-close="handleFormDialogClose">
      ...
      <span slot="footer" class="dialog-footer">
          ...
          <el-button type="primary" @click="handleFormDialogClose">确 定</el-button>
      </span>
  </el-dialog>
...
export default {
  ...
  methods: {
    ...
    handleFormDialogClose(){
      // 获取表单数据
      let formWindow = this.$refs.formIframe.contentWindow;
