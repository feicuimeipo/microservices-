...
export default {
  ...
  methods: {
    ...
    handleOpen(instId,defId,subject,nodeId) {
        ...
        //根据流程实例ID获取当前任务节点审批人
        let this_ =this;
        req.get(req.getContext().bpmRunTime + '/runtime/instance/v1/getExcutorNameByInstId?instId='+instId).then(res => {
          res = res.data;
          if(res){
            //任务管理催办
            if(nodeId){
        ...
        //根据流程定义id获取流程信息
        req.get(req.getContext().bpmModel + '/flow/def/v1/defGet?defId='+defId).then(res => {
          res = res.data;
    ...
    confirm() {
        ...
        const this_ = this;
        req.post(req.getContext().bpmRunTime + '/runtime/instance/v1/sendUrgentByInstId', data).then(response => {
