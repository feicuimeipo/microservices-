              ...
              <el-table-column label="菜单类型" width="140">
                <template slot-scope="scope">
                  <ht-select
                    ...
                    @change="workspaceChangeType($event, scope.row)"
                    placeholder="请选择模块名称"
                    :validate="{ required: true }"
                  ></ht-select>
                </template>
              ...
              <el-table-column label="显示方式" width="300">
                <template slot-scope="scope">
                  <el-radio
                    ...
                    label="flow"
                    border
                    @change="changeshowType($event, scope.row)"
                    >流程中心</el-radio
                  >
                  <el-radio
                    v-model="scope.row.showType"
...
export default {
  ...
  methods: {
    ...
    workspaceChangeType(code, item) {
      debugger;
      var data = this.getUrl(code);
      item.url = data.url;
      item.name = data.name;
    },
    changeshowType(value, item) {
      if (value == "flow") {
        var data = this.getUrl(item.code);
        item.url = data.url;
      } else {
        item.url =
