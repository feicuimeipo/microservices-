                ...
                <el-row type="flex" justify="space-around">
                  <el-col :span="4.8">
                    <i
                      ...
                      @click="handleCommand({ command: 'edit', row: item })"
                    >
                      <font class="li">编辑</font>
                    </i>
                  </el-col>
                  <el-col :span="4.8" v-if="item.type == 'weChatWork' || item.type == 'dingtalk'">
                    <i class="el-icon-upload2" style=" cursor: pointer " @click="syncUser(item)">
                      <font class="li">上传通讯录</font>
                    </i>
                  </el-col>
                  <el-col :span="4.8" v-if="item.type == 'weChatWork'">
                    <i class="el-icon-download" style=" cursor: pointer " @click="pullUser(item)">
                      <font class="li">拉取通讯录</font>
                    </i>
                  </el-col>
                  <el-col :span="4.8" v-if="item.type == 'weChatWork' || item.type == 'dingtalk'">
                    <i
                      ...
                      style=" cursor: pointer "
                      @click="handleCommand({ command: 'setAgent', row: item })"
                    >
                      <font class="li">配置应用</font>
                    </i>
                  </el-col>
                  <el-col :span="4.8" v-if="item.type == 'weChatOffAcc'">
                    <i class="el-icon-share" style=" cursor: pointer " @click="getMenu(item)">
                      <font class="li">生成菜单</font>
                    </i>
                  </el-col>
                  <el-col :span="4.8" v-if="item.type == 'weChatOffAcc'">
                    <i
                      class="el-icon-setting"
                      style=" cursor: pointer "
                      @click="openTempMsgIdDialog(item)"
                    >
                      <font class="li">设置模板消息ID</font>
                    </i>
                  </el-col>
                  <el-col :span="4.8">
                    <i
                      ...
                      style=" cursor: pointer "
                      @click="handleCommand({ command: 'del', row: item })"
                    >
                      <font class="li">删除</font>
                    </i>
                  </el-col>
                </el-row>
              </div>
        ...
        sys.syncUser(item.id).then(
          data => {
            if (data.state) {
              ...
              this.$message.error(data.message || "同步用户失败");
            }
          },
          error => {
            this.$message.error(error || "同步用户失败");
          }
        );
      }).catch(()=>{});;
    },
        ...
        sys.pullUser(item.id).then(
          data => {
            if (data.state) {
              ...
              this.$message.error(data.message || "拉取用户失败");
            }
          },
          error => {
            this.$message.error(error || "拉取用户失败");
          }
        );
      }).catch(()=>{});;
    },
