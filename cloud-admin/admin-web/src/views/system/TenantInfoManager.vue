     ...
     <el-tabs type="card" v-model="currentTabName">
          <el-tab-pane label="基本信息" name="basic">
            ...
            <el-form v-form data-vv-scope="tenantManageForm">
              <ht-form-item label="租户类型" label-width="120px" v-if="tenantId">
                <ht-select
                    class="ht"
              ...
              <ht-form-item label="租户名称" label-width="120px">
                <ht-input v-model="tenantManage.name" :validate="{required:true,regex: { exp: '^[\\s\\S]{1,50}$',message: '内容超出输入限制'}}" autocomplete="off" />
                <el-tooltip
                        class="item"
                        effect="dark"
                        content="租户名称会作为登录页的公司名称"
                        placement="right-start"
                >
                    <span class="el-icon-question" style="margin-left: 10px"></span>
                </el-tooltip>
              </ht-form-item>
              <ht-form-item label="租户别名" label-width="120px">
                <ht-input v-model="tenantManage.code"
                  autocomplete="off"
                  :validate="{
                    regex: {
                      exp: '^[a-z]{0,10}\\d{0,3}$',
                      message: '租户别名只能包含小写字母和数字，且必须以小写字母开头，小写字母最多10个，数字最多3个'
                    },
                    required: true,
                ...
                <el-tooltip
                        class="item"
                        effect="dark"
                        content="租户别名只能包含小写字母和数字，且必须以小写字母开头，小写字母最多10个，数字最多3个"
                        placement="right-start"
                >
                    <span class="el-icon-question" style="margin-left: 10px"></span>
                </el-tooltip>
              </ht-form-item>
              <ht-form-item label="租户简称" label-width="120px">
                <ht-input v-model="tenantManage.shorthand" validate="required"/>
              </ht-form-item>
              ...
              <ht-form-item label="描述" label-width="120px">
                <ht-input type="textarea" v-model="tenantManage.desc" placeholder="请输入类型说明" />
              </ht-form-item>
            </el-form>
          </el-tab-pane>
...
export default {
  ...
  data() {
    return {
      ...
      dialogVisible: false,
      tenantManage: {status:'draft',code:""},
      curSelectTenantId: '',
      tenantTypeList: [],
      tenantParam: {}, //当前租户的参数值
      tenantParams: [], //系统的所有组合类型参数
      saveBtnStyle: 'float: right;z-index: 15000;position: fixed;top:8px;right:15px',
  ...
  mounted() {
    ...
    if(!this.tenantId){
      this.saveBtnStyle = 'float: right;z-index: 15000;position: fixed;top:100px;right:15px'
