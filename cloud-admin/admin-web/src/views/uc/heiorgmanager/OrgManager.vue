<template>
  <div class="fullheight">
            ...
            <ht-table-column label="操作" width="150">
              <template v-slot="{row}">
                <el-dropdown
                    ...
                    @click="orgUserHandleCommand({row:row,command:'toUnderUserPage'})"
                    v-if="orgManagerNow.userPerms && orgManagerNow.userPerms.indexOf('edit')>=0"
                >
              <span>
                <i class="el-icon-user"></i>下属管理
              </span>
                  <el-dropdown-menu slot="dropdown">
                    <el-dropdown-item icon="el-icon-s-comment" :command="{row:row,command:'setIsMaster'}" v-if="row.isMaster=='0'">设置主组织</el-dropdown-item>
                    <el-dropdown-item icon="el-icon-s-comment" :command="{row:row,command:'setIsMaster'}" v-if="row.isMaster=='1'">取消主组织</el-dropdown-item>
    ...
    <!-- 直接下属管理组件   -->
    <under-user-table :org-id="org.id" :user-id="selectOrgUserAccount" ref="underUserTable"></under-user-table>
  </div>
</template>
