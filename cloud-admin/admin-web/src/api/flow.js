...
const model = req.getContext().bpmModel;
const runtime = req.getContext().bpmRunTime;
...
export default {
  ...
  getVarTree(defId){
      return new Promise((resolve, reject) => {
        req.post(req.getContext().bpmModel + "/flow/node/v1/varTree", {defId: defId, removeSub: true}).then(resp => {
          resolve(resp.data);
        },error => {
          reject(error);
        })
